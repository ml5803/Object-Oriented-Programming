...
Polynomial::Polynomial(const vector<int>& vecCoeff = {0}):lstCoeff(nullptr), degree(0){
    size_t counter = 0;
    //find first non-zero
...
Polynomial::Polynomial(const Polynomial& rhs):lstCoeff(new Node(rhs.lstCoeff->data,nullptr)), degree(rhs.degree){
...
Polynomial& Polynomial::operator+=(const Polynomial& rhs){
    ...
    //just before the end of polynomial
    ...
    if(myNode->next == nullptr && otherNode->next == nullptr){
        this->normalize(lstCoeff,lstCoeff->next);
        return *this;
    }
    ...
    this->Polynomial::normalize(lstCoeff,lstCoeff->next);
    //if this longer, do nothing
    return *this;
}

string Polynomial::makeDisplay(Node* currNode, int deg) const{
    string stringVal = "";
...
void Polynomial::normalize(Node* prev, Node* curr){
    if (curr == nullptr){
        return;
    }
    //last item is 0, kill node
    if(curr->data == 0 && curr->next == nullptr){
        delete curr;
        prev->next = nullptr;
    }else{
        Polynomial::normalize(curr->next, curr->next->next);
    }
}
